...
import com.gan.api.admin.param.CarouselAddParam;
import com.gan.api.admin.param.CarouselEditParam;
...
public class GanAdminCarouselAPI {

    private static final Logger logger = LoggerFactory.getLogger(GanAdminCarouselAPI.class);

    @Resource
    GanCarouselService ganCarouselService;
    ...
    public Result list(@RequestParam(required = false) @ApiParam(value = "页码") Integer pageNumber,
        ...
        return ResultGenerator.genSuccessResult(ganCarouselService.getCarouselPage(pageUtil));
    }
    ...
    public Result save(@RequestBody @Valid CarouselAddParam carouselAddParam, @TokenToAdminUser AdminUserToken adminUser) {
        ...
        String result = ganCarouselService.saveCarousel(carousel);
    ...
    public Result update(@RequestBody CarouselEditParam carouselEditParam, @TokenToAdminUser AdminUserToken adminUser) {
        ...
        String result = ganCarouselService.updateCarousel(carousel);
        if (ServiceResultEnum.SUCCESS.getResult().equals(result)) {
            return ResultGenerator.genSuccessResult();
        } else {
            return ResultGenerator.genFailResult(result);
    ...
    public Result info(@PathVariable("id") Integer id, @TokenToAdminUser AdminUserToken adminUser) {
        logger.info("adminUser:{}", adminUser.toString());
        Carousel carousel = ganCarouselService.getCarouselById(id);
        if (carousel == null) {
    ...
    public Result delete(@RequestBody BatchIdParam batchIdParam, @TokenToAdminUser AdminUserToken adminUser) {
        ...
        if (ganCarouselService.deleteBatch(batchIdParam.getIds())) {
            return ResultGenerator.genSuccessResult();
        } else {
